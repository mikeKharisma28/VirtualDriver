...
public class RaceTrackOneButtonsScript : MonoBehaviour 
{
    ...
    public Text displayConesHitCounter;
    ...
    void Start()
    {
        ...
        isPaused = true;
        if (File.Exists(Application.dataPath + "/Resources/FastestTimeTrackOneHolder.txt"))
        {
            //do nothing
        }
        else
        {
            using (StreamWriter selectedPathString = new StreamWriter(Application.dataPath + "/Resources/FastestTimeTrackOneHolder.txt", false))
            {
        ...
        StreamReader fileReader = new StreamReader(Application.dataPath + "/Resources/FastestTimeTrackOneHolder.txt");
        using (fileReader)
        {
    ...
    void Update()
    {
        displayConesHitCounter.text = (Mathf.Round(((float)ConesRigidCollision.s_conesHitCounter / 475.0f) * 100.0f)).ToString() + " % of 475 Cones were hit";
